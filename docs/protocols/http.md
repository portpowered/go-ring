...
`GetRecordingShareURL` calls Python's legacy `/clients_api/dings/{id}/share/play` and parses the returned `url`. `GetSnapshot` follows Python's legacy trigger/poll sequence: POST `/clients_api/snapshots/timestamps` with one `doorbot_ids` entry, poll for a timestamp newer than the trigger time, then GET `/clients_api/snapshots/image/{id}`. The Go call returns bounded image bytes, timestamp, and content type; it does not write a file. Both routes have shared synthetic replay coverage but no matching successful C1 response. C1's separate `app-snaps.ring.com/snapshots/next/{id}` requests remain an unverified profile and are not silently substituted.
