...
|---|---|---|---|---|
| Basic device inventory | [`tests/test_ring.py::test_basic_attributes`](../reference/python-ring-doorbell/tests/test_ring.py) expects chime, doorbell, shared doorbell, camera, and other families from the legacy inventory response. | [`device-list.json`](../tests/replay/fixtures/recordings/http/device-list.json) is one v3 inventory containing a captured PTZ indoor camera. | [`TestRecordedDeviceListAndGetDevice`](../tests/replay/client_recorded_devices_test.go) checks the captured camera; [`TestGetAllDevicesUniformMetadataAndDefaultsAcrossFamilies`](../pkg/ring/client_public_workflows_test.go) exercises public interface getters using existing Go fixture values. | The fixture-based mapping is synthetic and does not claim all-family v3 wire evidence. Unfamiliar kinds are retained as generic devices with their kind/family metadata; no capabilities are inferred. |
| Stickup camera attributes | [`tests/test_ring.py::test_stickup_cam_attributes`](../reference/python-ring-doorbell/tests/test_ring.py) checks kind/model/capabilities. | [`device-list.json`](../tests/replay/fixtures/recordings/http/device-list.json) contains `stickup_cam_mini_ptz_v1` and its captured attributes. | [`TestRecordedDeviceListAndGetDevice`](../tests/replay/client_recorded_devices_test.go); Python model adapter: [`test_shared_recordings.py`](../tools/reference-replay/test_shared_recordings.py). | Adapter maps the recorded kind into Python's existing family index to compare model properties. It does not claim Python's legacy request supports the v3 route. |
| Motion detection | [`tests/test_ring.py::test_motion_detection_enable`](../reference/python-ring-doorbell/tests/test_ring.py) guards the model using cached device settings, then sends a PATCH. | [`device-settings-get.json`](../tests/replay/fixtures/recordings/http/device-settings-get.json) supplies the current motion flag; [`device-settings-patch.json`](../tests/replay/fixtures/recordings/http/device-settings-patch.json) captures the same PATCH path, method, and JSON body as the Python call. | [`TestPortableLegacyHTTPControls/doorbell-motion-on`](../tests/replay/legacy_controls_portable_test.go) and captured settings replay exercise Go's actual PATCH. | Python adapter maps the captured v3 `motion_settings.motion_detection_enabled` value into the legacy cache field, then replays the recorded PATCH response through Python `Auth`. Route, query, and JSON body match; request headers are not compared by this adapter. |
| Siren control | [`tests/test_ring.py::test_stickup_cam_controls`](../reference/python-ring-doorbell/tests/test_ring.py) sends siren off and on commands. | [`siren-off.json`](../tests/replay/fixtures/recordings/http/siren-off.json) matches Python's off request. [`siren-on.json`](../tests/replay/fixtures/recordings/http/siren-on.json) captures the on path with no query pair. | `Client.SetSiren` is covered by [`TestRecordedSettingsAndSirenAcrossRegions`](../tests/replay/client_recorded_settings_test.go). | The adapter replays the off call through Python `Auth`; a separate request-shape test shows Python's on call adds `duration=30`, which this capture does not contain. The response's duration does not establish the request query, so full on-request parity remains unverified. |
| History | [`tests/test_ring.py::test_doorbell_attributes`](../reference/python-ring-doorbell/tests/test_ring.py) checks legacy per-device history results. | [`device-timeline.json`](../tests/replay/fixtures/recordings/http/device-timeline.json) and [`history-devices.json`](../tests/replay/fixtures/recordings/http/history-devices.json) use EVM routes and response envelopes. | [`TestGetDeviceHistory_Success`](../pkg/ring/client_recordings_test.go) exercises Go's existing history method against its own legacy fixture. | Python's `/clients_api/doorbots/{id}/history` and its array results are distinct from C1's timeline and grouped-feed contracts; the adapter asserts the gap without feeding EVM data into the legacy parser. |
| Device detail | Python attribute cases read cached inventory objects; the current Go `GetDevice` also searches its list response. | [`device-detail.json`](../tests/replay/fixtures/recordings/http/device-detail.json) records `GET /device_info/v3/devices/{device_id}`. | `GetDevice` is covered via [`device-list.json`](../tests/replay/fixtures/recordings/http/device-list.json). | The detail recording route has no current public Go method and is not counted as supported-route coverage. |
| Snapshot and recording share URL | Python `async_get_snapshot` triggers/polls a timestamp then downloads image bytes; `async_recording_url` parses a share URL. | C1's `app-snaps.ring.com/snapshots/next/{id}` requests lack successful responses; no C1 share URL response exists. | [`TestPortableSnapshotFreshnessAndImage`](../tests/replay/snapshot_portable_test.go) and [`TestPortableRecordingShareURL`](../tests/replay/media_failure_portable_test.go) replay the shared synthetic media values and Python legacy routes. | Go returns bounded snapshot bytes and timestamp, not a file; the C1 app-snaps profile remains unverified and separate. |
...
[`tests/replay/client_recorded_devices_test.go`](../tests/replay/client_recorded_devices_test.go)
and runs with `go test ./tests/replay`. Unit configuration coverage is in
[`pkg/ring/client_endpoints_test.go`](../pkg/ring/client_endpoints_test.go).
